...
class Visit {
    constructor(id, patient, doctor, title, description = '', priority, status) {
        this._id = id;
    ...
    editMenu() {
        ...
        editMenu.innerHTML = `<ul>
            <li><a class="visit-edit-menu-item">${this._status === 'Визит завершен' ? 'Открыть' : 'Завершить'}</a></li>
    ...
    editCard(formDefault) {
        ...
        new formDefault(this._id, this._status).render();
...
class VisitCardio extends Visit {
    ...
    editStatus() {
        const content = {
            ...
            weightIndex: this._massIndex,
            disease: this._diseases,
        };
        super.editStatus(content);
    }
}
...
navbar.addEventListener('click', (event) => {
    ...
    if (event.target.id === 'login-btn') {
        ...
        const modal = document.getElementById('modal-wrapper');  /*Modal Window*/
                                    ...
                                    if (dataArr.length > 0) {
                                        dataArr.forEach(function (item) {
                                            if (item.doctor === 'dentist') {
                                                ...
                                                const idD = item.id;
                                                const statusD = item.status;
                                                const visitDentist = new VisitDentist(dateD, idD, nameD, "Стоматолог", titleD, descriptionD, priorityD, statusD);
                                            ...
                                            if (item.doctor === 'cardiologist') {
                                                ...
                                                const ageC = item.age;
                                                const idC = item.id;
                                                const statusC = item.status;
                                                const pressureC = item.pressure;
                                                const weightIndexC = item.weightIndex;
                                                const diseaseC = item.disease;
                                                const visitCardiolog = new VisitCardio(pressureC, weightIndexC, diseaseC, ageC, idC, nameC, "Кардиолог", titleC, descriptionC, priorityC, statusC);
                                            ...
                                            if (item.doctor === 'therapist') {
                                                ...
                                                const ageT = item.age;
                                                const idT = item.id;
                                                const statusT = item.status;
                                                const visitTherapist = new VisitTherapist(ageT, idT, nameT, "Терапевт", titleT, descriptionT, priorityT, statusT);
...
class visitFormDentist extends visitForm {
    constructor(id, status = 'В процессе') {
        super(id);
        this._status = status;
    }
    ...
    render() {
        ...
        const requestOptions = super.checkRequestType(this._id, this._status);
            ...
            axios(authOptions)
                .then(function (response) {
                    ...
                    if (response.status >= 200 && response.status < 300) {
                        ...
                        const visitDentist = new VisitDentist(date, dataId, name, "Стоматолог", title, description, priority, status);
                        const cardsContainer = document.getElementById('cards-container');
                        visitDentist.render(cardsContainer);
            ...
            axios(authOptions)
                .then(function (response) {
                    ...
                    if (response.status >= 200 && response.status < 300) {
                        ...
                        const visitTherapist = new VisitTherapist(age, dataId, name, "Терапевт", title, description, priority, status);
                        const cardsContainer = document.getElementById('cards-container');
                        visitTherapist.render(cardsContainer);
                        requestOptions.ifPut();
                    } else {
...
class visitFormCardiolog extends visitForm {
    constructor(id, status = 'В процессе', ...args) {
        super(...args);
        this._status = status;
    }
            ...
            axios(authOptions)
                .then(function (response) {
                    ...
                    if (response.status >= 200 && response.status < 300) {
                        ...
                        const visitCardiolog = new VisitCardio(pressure, weightIndex, disease, age, dataId, name, "Кардиолог", title, description, priority, status);
                        const cardsContainer = document.getElementById('cards-container');
                        visitCardiolog.render(cardsContainer);
                        requestOptions.ifPut();
                    } else {
